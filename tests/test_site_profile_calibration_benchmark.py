...
from services.benchmarks.site_profile_calibration import format_result, run_calibration


def test_run_site_profile_calibration(tmp_path: Path) -> None:
    ...
    assert all(run.recommended_batch_size >= 64 for run in result.runs)
    assert "recommended_min" in format_result(result)
