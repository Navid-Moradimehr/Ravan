...
@dataclass(frozen=True)
class EndToEndPipelineBenchmarkResult:
    csv_path: str
    wire_format: str
    events: int
    invalid_events: int
...
def run_benchmark(
...
) -> EndToEndPipelineBenchmarkResult:
    ...
    for index in range(total_iterations):
        ...
        stage_started = time.perf_counter()
        window = windows.get(runtime_event.device_id)
        if window is None:
            window = RollingWindowState(maxlen=window_limit)
            windows[runtime_event.device_id] = window
        ...
        stage_started = time.perf_counter()
        payload = serialize_payload(processed, wire_format=wire_format)
        roundtrip = deserialize_payload(payload, wire_format=wire_format)
        wire_roundtrip_elapsed_ms = (time.perf_counter() - stage_started) * 1000.0
        ...
        if index >= warmup_events:
            ...
            wire_roundtrip_ms.append(wire_roundtrip_elapsed_ms)
            payload_bytes += len(payload)
            roundtrip_bytes += len(serialize_payload(roundtrip, wire_format=wire_format))
    ...
    return EndToEndPipelineBenchmarkResult(
        ...
        stage_breakdown=(
            ...
            _stage_result("wire_roundtrip", wire_roundtrip_ms),
        ),
    )


def format_result(result: EndToEndPipelineBenchmarkResult) -> str:
    lines = [
        ...
        f"payload_bytes={result.payload_bytes}",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Benchmark the end-to-end industrial pipeline with selectable wire format.")
    ...
    parser.add_argument("--wire-format", default="json", choices=("json", "msgpack"))
    return parser
