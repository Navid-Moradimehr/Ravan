...
async def send_message(thread_id: str, request: MessageRequest) -> dict[str, Any]:
    ...
    assistant_message = store.append_message(thread_id, actor_id=actor_id, role="assistant", content=answer, metadata={"links": links, "provider": "gateway" if answer else "deterministic", "tool_result": tool_result})
    if request.content.lower().startswith(("remember that", "i prefer", "always ")):
        ...
        candidate = None
    return {"user_message": user_message, "assistant_message": assistant_message, "links": links, "tool_result": tool_result, "memory_candidate": candidate}
