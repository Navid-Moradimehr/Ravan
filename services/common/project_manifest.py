...
class ProjectManifest:
    ...
    def _write_text(path: Path, content: str) -> Path:
        ...
        # Keep generated shell and YAML artifacts portable when created on
        # Windows. Python's default newline translation would make .sh files
        # fail under Bash on Linux hosts.
    ...
    def _render_systemd_unit(self, site_id: str) -> str:
        ...
        return "\n".join(
            [
                ...
                f"Environment=PYTHONPATH={site_root}/runtime",
                f"Environment=DATASTREAM_PID_DIR={site_root}/.datastream",
                f"WorkingDirectory={site_root}/runtime",
                f"ExecStart={site_root}/runtime/.venv/bin/python -m services.cli.datastreamd supervise --site-profile {site_root}/site-profile.yaml",
                "",
                "[Install]",
    ...
    def _render_systemd_readme(self, site_id: str) -> str:
        return "\n".join(
            [
                ...
                "3. Reload systemd and enable the unit. The unit runs the foreground supervisor, which owns child processes and performs bounded restarts.",
                "4. The installer copies the sibling runtime bundle and creates runtime/.venv. Set RAVAN_SKIP_PIP_INSTALL=1 only when dependencies are already installed from an approved offline source.",
                "5. Kafka, TimescaleDB, Flink, and other infrastructure remain external dependencies for host-native mode; use the Compose bundle for the complete single-host stack.",
                "",
    ...
    def _render_systemd_install(self, site_id: str) -> str:
        ...
        return "\n".join(
            [
                ...
                'PYTHON_BIN="${RAVAN_PYTHON:-python3}"',
                "",
                'if [ -n "${5:-}" ]; then RUNTIME_DIR="$5"; else RUNTIME_DIR="$(cd "${SOURCE_DIR}/../../runtime" && pwd)"; fi',
                'if ! command -v "${PYTHON_BIN}" >/dev/null 2>&1; then echo "Ravan requires Python 3.10+; set RAVAN_PYTHON to the approved interpreter." >&2; exit 1; fi',
                '"${PYTHON_BIN}" -c "import sys; assert sys.version_info >= (3, 10), sys.version" || { echo "Ravan requires Python 3.10+" >&2; exit 1; }',
                'if [ ! -f "${RUNTIME_DIR}/requirements.txt" ]; then echo "Runtime bundle not found: ${RUNTIME_DIR}" >&2; exit 1; fi',
                'install -d -m 0755 "${TARGET_DIR}"',
                'install -d -m 0755 "${TARGET_DIR}/env"',
                'install -d -m 0755 "${TARGET_DIR}/systemd"',
                'install -d -m 0755 "${TARGET_DIR}/runtime"',
                'cp -a "${RUNTIME_DIR}/." "${TARGET_DIR}/runtime/"',
                '"${PYTHON_BIN}" -m venv "${TARGET_DIR}/runtime/.venv"',
                'if [ "${RAVAN_SKIP_PIP_INSTALL:-0}" != "1" ]; then "${TARGET_DIR}/runtime/.venv/bin/python" -m pip install --requirement "${TARGET_DIR}/runtime/requirements.txt"; fi',
    ...
    def _render_systemd_uninstall(self, site_id: str) -> str:
        ...
        return "\n".join(
            [
                ...
                "systemctl daemon-reload",
                'TARGET_DIR="/etc/datastream/' + f"{self.project_id}/{site_id}" + '"',
                'if [ "${2:-0}" = "1" ]; then rm -rf "${TARGET_DIR}"; echo "Removed ${UNIT_NAME} and purged ${TARGET_DIR}"; else echo "Removed ${UNIT_NAME}; preserved ${TARGET_DIR} for rollback and data retention"; fi',
                "",
            ]
        )
    ...
    def _render_windows_readme(self, site_id: str) -> str:
        return "\n".join(
            [
                ...
                "3. The installer copies the sibling runtime bundle and creates runtime/.venv. Set RAVAN_SKIP_PIP_INSTALL=1 only when dependencies are already installed from an approved offline source.",
                "4. Start the datastreamd service and verify datastreamctl status. The service runs the foreground supervisor and performs bounded restarts.",
                "5. Kafka, TimescaleDB, Flink, and other infrastructure remain external dependencies for host-native mode; use the Compose bundle for the complete single-host stack.",
                "",
    ...
    def _render_windows_cmd(self, command: str, extra_args: str = "") -> str:
        return "\n".join(
            [
                "@echo off",
                "setlocal",
                "set \"RAVAN_RUNTIME_DIR=%~dp0..\\runtime\"",
                "if exist \"%RAVAN_RUNTIME_DIR%\\.venv\\Scripts\\python.exe\" set \"RAVAN_PYTHON=%RAVAN_RUNTIME_DIR%\\.venv\\Scripts\\python.exe\"",
                "if not defined RAVAN_PYTHON set \"RAVAN_PYTHON=python\"",
                "set \"PYTHONPATH=%RAVAN_RUNTIME_DIR%;%PYTHONPATH%\"",
                "cd /d \"%RAVAN_RUNTIME_DIR%\"",
                f'"%RAVAN_PYTHON%" -m {command} {extra_args} %*',
                "endlocal",
                "",
            ]
        )
    ...
    def _render_windows_install(self, site_id: str) -> str:
        ...
        return "\n".join(
            [
                ...
                "",
                "if ([string]::IsNullOrWhiteSpace($RuntimeDir)) { $RuntimeDir = (Resolve-Path (Join-Path $SourceDir '..\\..\\runtime')).Path }",
                "if (-not (Test-Path (Join-Path $RuntimeDir 'requirements.txt'))) { throw ('Runtime bundle not found: ' + $RuntimeDir) }",
                ...
                "Copy-Item (Join-Path $RuntimeDir '*') $runtimeTarget -Recurse -Force",
                "$hostPython = (Get-Command python -ErrorAction Stop).Source",
                "& $hostPython -c \"import sys; assert sys.version_info >= (3, 10), sys.version\"",
                "& $hostPython -m venv (Join-Path $runtimeTarget '.venv')",
                "$pythonExe = Join-Path $runtimeTarget '.venv\\Scripts\\python.exe'",
                "if ($env:RAVAN_SKIP_PIP_INSTALL -ne '1') { & $pythonExe -m pip install --requirement (Join-Path $runtimeTarget 'requirements.txt') }",
                ...
                "",
                "$serviceLauncher = Join-Path $binDir 'datastreamd-service.cmd'",
                "$serviceContent = @\"",
                "@echo off",
                "set \"PYTHONPATH=$runtimeTarget\"",
                "set \"DATASTREAM_PID_DIR=$(Join-Path $TargetDir '.datastream')\"",
                "cd /d \"$runtimeTarget\"",
                "\"$pythonExe\" -m services.cli.datastreamd supervise --site-profile \"$(Join-Path $TargetDir 'site-profile.yaml')\"",
                "\"@",
                "Set-Content -Path $serviceLauncher -Value $serviceContent -Encoding ASCII",
                "$binaryPath = '{0} /d /c \"\"{1}\"\"' -f $env:ComSpec, $serviceLauncher",
