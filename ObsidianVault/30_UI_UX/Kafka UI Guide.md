...
- inspect message keys and payloads to understand the stream shape
...
The broker metrics page is now wired to the local broker's JMX endpoint. In the compose stack the Kafka broker exposes JMX on port `9997` and Kafka UI is configured to read broker metrics from that endpoint. That keeps the metrics tab from returning an empty response and throwing a browser JSON parse error. If you rebuild the stack and the metrics page is still empty, check that the broker container started with the JMX options and that port `19097` is reachable from the host.
