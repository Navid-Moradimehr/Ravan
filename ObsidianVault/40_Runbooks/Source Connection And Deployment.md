...
The 2026-07-16 Docker verification rebuilt API, edge ingest, and dashboard successfully. A post-restart HTTP Push replay returned `duplicate`, proving the idempotency record survives API process replacement. OPC UA browse selections now persist through `/api/v1/connections/{id}/opcua/browse-selection`, and signed connections reject expired or unreadable client material at the edge. Sparkplug birth/death messages update source health and create canonical lifecycle state events; an explicit `request_rebirth_on_connect` option can publish a Tahu Node Control/Rebirth command. Focused connector tests and the UI build passed; host-side backup CLI availability remains a separate release-gate prerequisite.
