...
The 2026-07-16 Docker verification rebuilt API, edge ingest, and dashboard successfully. A post-restart HTTP Push replay returned `duplicate`, proving the idempotency record survives API process replacement. Focused connector tests and the UI build passed; host-side backup CLI availability remains a separate release-gate prerequisite.
