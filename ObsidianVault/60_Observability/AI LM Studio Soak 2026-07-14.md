...
The definitive five-minute run sent 900 events from three simulated source
identities using OPC UA, MQTT, and Modbus protocol labels. It validated the
complete path:
...
The latest Kafka record contained 22 warning evidence events for
`plant-a/Pump-01/Vibration`, `report_type=anomaly`,
`trigger_reason=sustained_anomaly`, and model `openai/gpt-oss-20b`. The report
was persisted to `ai_enriched` by the historian fan-out. After verification,
the temporary soak policy was restored to hourly scheduled reporting with
sustained-anomaly reporting disabled by default.
