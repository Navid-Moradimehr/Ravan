...
The live panels are fed by the API service websocket layer. The alarms view only changes when the historian has new warning or critical rows, while the raw events view redraws whenever the historian stream snapshot changes for the selected table. That means the page is not inventing data in the browser; it is rendering the current historian state coming from the backend.
