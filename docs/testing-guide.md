...
11. Backup drill: run `datastreamctl backup-drill --restore-db stream_engine_restore_demo`. The drill resets the disposable restore target first, then verifies row counts and Timescale hypertables on the restored database.
