...
- Partially implemented: full OPC UA trust-store provisioning/renewal and vendor-specific gateway compatibility, OAuth2/mTLS diagnostics beyond connector execution, and deeper source-to-asset mapping UI. Sparkplug command/rebirth requests are implemented as an explicit opt-in; vendor-specific command semantics remain gateway-owned.
