# Ravan Documentation

This directory contains operator and user documentation released with Ravan.
It intentionally excludes internal planning, benchmark notes, implementation
logs, and the maintainer Obsidian vault.

## Start Here

- [Platform capabilities](app-functionality.md)
- [Self-host installation](self-host-install-guide.md)
- [Linux Site Server installer](linux-site-server-install.md)
- [Desktop Operator workflow](operator-desktop-guide.md)
- [Docker operator guide](docker-operator-guide.md)
- [Connect industrial sources](source-connection-walkthrough.md)
- [Historian and dashboards](historian-guide.md)
- [Observability](observability-walkthrough.md)
- [Latest benchmark results](latest-benchmark-results.md)

## Optional Capabilities

- [AI provider configuration](ai-provider-configuration.md)
- [Lakehouse and S3](lakehouse-and-s3-guide.md)
- [Multi-site rollout](multi-site-rollout.md)
- [World-model data foundation](world-model-data-foundation.md)

## Operator References

- [Secrets boundary](self-hosted-secrets.md)
- [Flink Kubernetes Operator runbook](flink-operator-runbook.md)
- [Local Kubernetes rehearsal](local-kubernetes-rehearsal.md)
- [Local validation and release checks](deployment-targets.md)
- [Release and update operations](update-and-release-operations.md)
- [Release identity](release-identity.md)
- [Deployment targets](deployment-targets.md)
