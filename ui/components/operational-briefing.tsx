...
const statusStyle = {
  ...
  recovering: "border-info/30 bg-info/10 text-info",
  unknown: "border-border bg-surface-2 text-text-secondary",
};
    ...
    <article className="space-y-5" aria-label={`Operational briefing: ${briefing.headline}`}>
      <header className="overflow-hidden rounded-xl border border-border bg-surface-1">
        <div className="border-b border-border-subtle bg-surface-2/65 p-5">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="outline" className={cn("capitalize", statusStyle[briefing.situation_status])}>{briefing.situation_status}</Badge>
            <Badge variant="outline" className="capitalize">{job.report_type}</Badge>
            <span className="font-mono text-xs text-text-muted">{job.site_id}</span>
          </div>
          <h2 className="mt-4 text-balance font-heading text-2xl font-semibold leading-tight tracking-tight text-text-primary">{briefing.headline}</h2>
