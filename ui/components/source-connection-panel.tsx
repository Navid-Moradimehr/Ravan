...
function parseRegisterMap(text: string, defaultUnitId: string) {
  return text.split("\n").map((line) => {
    const trimmed = line.trim();
    if (!trimmed) return null;
    if (!trimmed.includes(",") && trimmed.includes(":")) {
      const [address, count] = trimmed.split(":").map((item) => item.trim());
      return { address: Number(address), tag: `register_${address}`, unit: "", scale: 1, offset: 0, unit_id: Number(defaultUnitId || 1), data_type: count === "1" ? "uint16" : "uint16", byte_order: "big", word_order: "big", count: Number(count || 1) };
    }
    const [address, tag, unit, scale, offset, unit_id, data_type, byte_order, word_order] = trimmed.split(",").map((item) => item.trim());
    return { address: Number(address), tag, unit, scale: Number(scale || 1), offset: Number(offset || 0), unit_id: Number(unit_id || defaultUnitId || 1), data_type: data_type || "uint16", byte_order: byte_order || "big", word_order: word_order || "big" };
  }).filter((item) => Boolean(item && Number.isFinite(Number(item.address)) && item.tag));
}
...
export function SourceConnectionPanel() {
  ...
  function payload() {
    ...
    else if (["modbus", "modbus_rtu"].includes(protocol)) config = { ...(protocol === "modbus" ? { asset_id: modbusAsset } : { port: rtuPort, baudrate: Number(baudrate), slave_id: Number(slaveId) }), registers: parseRegisterMap(registersText, protocol === "modbus_rtu" ? slaveId : "1") };
  ...
  return (
    ...
    <Card className={`app-card transition-colors ${editingId ? "border-accent/50 shadow-[0_0_0_1px_rgba(218,165,32,0.18)]" : ""}`}>
      ...
      <CardContent className="space-y-4 p-4">
        ...
        <div className={`${step === 1 ? "" : "hidden"} grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-4`}>
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-2 md:col-span-2 xl:col-span-4"><Button onClick={() => { if (!name || !siteId) return; setStep(2); }} disabled={!name || !siteId}><ChevronRight className="size-4" /> {editingId ? "Continue editing" : "Configure source"}</Button>{editingId ? <Button variant="outline" onClick={cancelEditing}>Cancel</Button> : null}</div>
        </div>
        <div className={`${step === 2 ? "" : "hidden"} space-y-3`}><div className="grid gap-3 md:grid-cols-3"><label className="space-y-1 text-xs text-text-secondary">Endpoint / URL<Input value={endpoint} onChange={(event) => setEndpoint(event.target.value)} placeholder={protocol === "http_push" ? "Generated after activation" : "opc.tcp://host:4840"} /></label><label className="space-y-1 text-xs text-text-secondary">Legacy credential reference<Input value={credentialRef} onChange={(event) => setCredentialRef(event.target.value)} placeholder="secret://plant-a/source" /></label><label className="space-y-1 text-xs text-text-secondary">Source ID<Input value={sourceId} onChange={(event) => setSourceId(event.target.value)} placeholder="Optional source identifier" /></label></div><div className="rounded-lg border border-border-subtle bg-surface-1 p-3"><div className="mb-2 flex items-center justify-between"><p className="text-sm font-medium">Protocol settings</p><HelpTip label="Protocol settings help" content="These fields become the edge connector definition. Use env://NAME, file://path, path://path, or secret://provider references. The platform reads referenced values only inside the runtime." /></div>
          ...
          {["modbus", "modbus_rtu"].includes(protocol) ? <div className="space-y-3">{protocol === "modbus" ? <label className="block space-y-1 text-xs text-text-secondary">Asset ID<Input value={modbusAsset} onChange={(event) => setModbusAsset(event.target.value)} placeholder="Pump-03" /></label> : null}<label className="block space-y-1 text-xs text-text-secondary">{protocol === "modbus_rtu" ? "Register map" : "Holding registers"}<textarea className="app-textarea min-h-20 w-full font-mono text-xs" value={registersText} onChange={(event) => setRegistersText(event.target.value)} placeholder="address,tag,unit,scale,offset,unit_id,data_type,byte_order,word_order&#10;0,Temperature,C,0.1,1,1,float32,big,big" /></label><p className="text-xs leading-5 text-text-secondary">Use uint16, int16, uint32, int32, float32, uint64, int64, float64, or bool. Multi-register values honor byte and word order. RTU keeps the same map across serial devices.</p></div> : null}
