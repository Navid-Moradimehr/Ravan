...
import { Archive, Bot, Brain, ChevronRight, History, LoaderCircle, MessageCircle, Mic, Pencil, Plus, RotateCcw, Send, Square, X } from "lucide-react";
...
function AssistantDrawerInner() {
  ...
  async function send() {
    ...
    try {
      ...
      let buffer = "";
      let completed: { user_message: AssistantMessage; assistant_message: AssistantMessage; action_preview?: ActionPreview | null; memory_candidate?: MemoryCandidate | null; questionnaire?: Questionnaire | null } | null = null;
      ...
      const finalResponse = completed as { user_message: AssistantMessage; assistant_message: AssistantMessage; action_preview?: ActionPreview | null; memory_candidate?: MemoryCandidate | null; questionnaire?: Questionnaire | null };
      ...
      setThreads((current) => current.map((item) => item.thread_id === thread.thread_id ? { ...item, updated_at: finalResponse.assistant_message.created_at } : item));
    ...
    <>
      ...
      {open ? <aside className="fixed inset-y-0 right-0 z-[51] flex w-full max-w-[min(30rem,100vw)] flex-col border-l border-border-subtle bg-surface-1 shadow-2xl" aria-label="Ravan Assistant">
        ...
        <div className={`flex-1 space-y-3 overflow-y-auto p-4 ${assistantView !== "chat" ? "hidden" : ""}`}>
          ...
          {busy ? <div className="flex items-center gap-2 text-xs text-text-muted"><LoaderCircle className="size-3.5 animate-spin" />Ravan is thinking and streaming a response…</div> : null}
