...
type Questionnaire = { question_id: string; status: string; questions: Array<{ key: string; label?: string; question: string; reason?: string; format_hint?: string; example?: string; type: string; options?: string[]; required?: boolean; lookup_kind?: string; help?: { summary?: string; details?: string; source?: string } }>; answers: Record<string, string>; draft?: Record<string, string>; validation_errors?: string[]; field_errors?: Record<string, string> };
type ToolStep = { tool: string; status: string; summary: string; retryable?: boolean };

function QuestionnaireFields({ questionnaire, answers, fieldErrors, expandedHelp, onAnswer, onToggleHelp }: { questionnaire: Questionnaire; answers: Record<string, string>; fieldErrors: Record<string, string>; expandedHelp: Record<string, boolean>; onAnswer: (key: string, value: string) => void; onToggleHelp: (key: string) => void }) {
  return <div className="mt-3 space-y-3">{questionnaire.questions.map((question) => {
    const helpOpen = Boolean(expandedHelp[question.key]);
    const fieldError = fieldErrors[question.key];
    const label = question.label || question.question;
    return <div key={question.key} className="space-y-1">
      <label className="block"><span className="mb-1 block text-xs font-medium text-text-primary">{label}{question.required ? " *" : ""}</span>{question.reason ? <span className="mb-1 block text-[0.68rem] leading-4 text-text-muted">{question.reason}</span> : null}
        {question.type === "choice" ? <select aria-label={label} value={answers[question.key] || ""} onChange={(event) => onAnswer(question.key, event.target.value)} className={`w-full rounded-lg border bg-surface-1 px-2 py-2 text-xs text-text-primary ${fieldError ? "border-destructive/60" : "border-border-subtle"}`}><option value="">Select an option</option>{question.options?.map((option) => <option key={option} value={option}>{option}</option>)}</select> : <input aria-label={label} value={answers[question.key] || ""} onChange={(event) => onAnswer(question.key, event.target.value)} placeholder={question.example || question.format_hint || "Enter a value"} className={`w-full rounded-lg border bg-surface-1 px-2 py-2 text-xs text-text-primary ${fieldError ? "border-destructive/60" : "border-border-subtle"}`} />}
      </label>
      {question.format_hint ? <p className="text-[0.68rem] leading-4 text-text-muted">Format: {question.format_hint}</p> : null}
      {fieldError ? <p role="alert" className="text-[0.68rem] leading-4 text-destructive">{fieldError}</p> : null}
      {question.help ? <><button type="button" className="text-[0.68rem] font-semibold text-accent hover:underline" aria-expanded={helpOpen} onClick={() => onToggleHelp(question.key)}>{helpOpen ? "Hide where to find this" : "Where to find this"}</button>{helpOpen ? <div className="rounded-md border border-border-subtle bg-surface-2 px-2.5 py-2 text-[0.68rem] leading-4 text-text-secondary"><p>{question.help.details || question.help.summary}</p>{question.help.source ? <p className="mt-1 text-text-muted">Source: {question.help.source.replace(/_/g, " ")}</p> : null}</div> : null}</> : null}
    </div>;
  })}</div>;
}
...
function AssistantDrawerInner() {
  ...
  const [questionnaireFieldErrors, setQuestionnaireFieldErrors] = useState<Record<string, string>>({});
  ...
  async function loadThread(threadId: string) {
    ...
    setQuestionnaireFieldErrors(pending?.field_errors || {});
  }
    ...
    try {
      ...
      const applyEvent = (raw: string) => {
        ...
        if (eventName === "token" && typeof payload.text === "string") {
          ...
          setThread((current) => current ? { ...current, messages: current.messages.map((message) => message.message_id === temporaryAssistant.message_id ? { ...message, metadata: { ...(message.metadata || {}), tool_steps: [...((message.metadata?.tool_steps as ToolStep[] | undefined) || []), payload as unknown as ToolStep] } } : message) } : current);
  ...
  async function answerQuestion() {
    ...
    setError(null);
    try {
      const response = await requestJson<{ assistant_message: AssistantMessage; questionnaire: Questionnaire; source_draft?: Record<string, string>; validation_errors?: string[]; field_errors?: Record<string, string> }>(`/api/assistant/threads/${thread.thread_id}/questions/${questionnaire.question_id}/answer`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setQuestionnaireFieldErrors(response.field_errors || response.questionnaire.field_errors || {});
      if (response.source_draft) {
        window.sessionStorage.setItem("ravan.assistant.sourceDraft", JSON.stringify(response.source_draft));
      }
    } catch (reason) {
    ...
    <>
      ...
      {open ? <aside role="dialog" aria-modal="true" tabIndex={-1} className="fixed inset-y-0 right-0 z-[51] flex min-h-0 w-full max-w-[min(30rem,100vw)] flex-col overscroll-contain border-l border-border-subtle bg-surface-1 shadow-2xl" aria-label="Ravan Assistant">
        ...
        <div className={`flex-1 space-y-3 overflow-y-auto p-4 ${assistantView !== "chat" ? "hidden" : ""}`}>
          ...
          {questionnaire ? <div className="rounded-xl border border-accent/30 bg-accent-subtle/40 p-4"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-accent">Source setup</p><p className="mt-2 text-xs leading-5 text-text-secondary">Answer the missing fields. Ravan will prepare a draft, but will not save credentials or activate ingestion.</p><QuestionnaireFields questionnaire={questionnaire} answers={questionAnswers} fieldErrors={questionnaireFieldErrors} expandedHelp={expandedQuestionHelp} onAnswer={(key, value) => { setQuestionAnswers((current) => ({ ...current, [key]: value })); setQuestionnaireErrors([]); setQuestionnaireFieldErrors((current) => ({ ...current, [key]: "" })); }} onToggleHelp={(key) => setExpandedQuestionHelp((current) => ({ ...current, [key]: !current[key] }))} /><Button className="mt-4 w-full" size="sm" onClick={() => void answerQuestion()} disabled={busy}>{busy ? "Validating source details…" : "Prepare source draft"}</Button>{questionnaireErrors.length > 0 ? <div role="alert" className="mt-3 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs leading-5 text-destructive"><p className="font-semibold">Source details need correction</p><ul className="mt-1 list-disc space-y-1 pl-4">{questionnaireErrors.map((message) => <li key={message}>{message}</li>)}</ul></div> : null}</div> : null}
