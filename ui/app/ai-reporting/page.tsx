    ...
    <DashboardFrame>
      ...
      <Tabs defaultValue="reports" className="space-y-4">
        ...
        <TabsContent value="reports" className="space-y-4">
          ...
          {reports.isError ? <BriefingFailure message={formatErrorMessage(reports.error)} /> : reports.isLoading ? <div aria-busy="true" aria-label="Loading operational briefings" className="grid gap-4 2xl:grid-cols-[21rem_minmax(0,1fr)]"><div className="rounded-xl border border-border-subtle bg-surface-1 p-3"><Skeleton className="mb-4 h-5 w-32 bg-surface-2" /><div className="space-y-3">{[1, 2, 3, 4, 5].map((item) => <div key={item} className="rounded-lg border border-border-subtle/70 p-3"><Skeleton className="h-4 w-4/5 bg-surface-2" /><Skeleton className="mt-2 h-3 w-2/5 bg-surface-2" /></div>)}</div></div><div className="rounded-xl border border-border-subtle bg-surface-1 p-5"><Skeleton className="h-6 w-3/5 bg-surface-2" /><Skeleton className="mt-3 h-4 w-1/3 bg-surface-2" /><div className="mt-8 space-y-3"><Skeleton className="h-24 w-full bg-surface-2" /><Skeleton className="h-24 w-full bg-surface-2" /><Skeleton className="h-16 w-4/5 bg-surface-2" /></div></div></div> : completed.length ? (
