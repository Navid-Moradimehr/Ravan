...
export async function DELETE(request: Request, context: { params: Promise<{ kpiId: string }> }) {
  try {
    const { kpiId } = await context.params;
    const authorization = request.headers.get("authorization");
    const response = await fetch(`${API_SERVICE_BASE}/api/v1/kpis/${encodeURIComponent(kpiId)}`, {
      method: "DELETE",
