...
export async function DELETE(request: Request, context: { params: Promise<{ hookId: string }> }) {
  try {
    const { hookId } = await context.params;
    const authorization = request.headers.get("authorization");
    const response = await fetch(`${API_SERVICE_BASE}/api/v1/webhooks/${encodeURIComponent(hookId)}`, {
      method: "DELETE",
