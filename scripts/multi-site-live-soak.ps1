...
# The current Compose file keeps edge-ingest behind the edge profile. The
# protocol simulators are separate maintainer fixtures and may not be present
# in a release Compose file, so do not make the soak depend on their names.
Invoke-Compose -Arguments @("--profile", "edge", "stop", "edge-ingest") | Out-Null
