param(
  ...
  [string]$RecoveryService = "edge-ingest",
  [switch]$IncludeApi
)
...
$available = @(docker compose -f $compose $profiles config --services | ForEach-Object { "$($_)".Trim() })
foreach ($service in $services) {
  if ($available -notcontains $service) { throw "Soak service '$service' is not defined in docker-compose.yml" }
}
